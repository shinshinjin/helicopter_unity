...
public class HelicopterMovement : MonoBehaviour
{
    ...
    public float HelicopterSpeed = -0.1f;
    public float GoFrontSpeed = 1.2f;
    public GameObject Propeller;
    public GameObject TailPropeller;
    public GameObject Missile;

    private HelicopterMove _input;
    private Rigidbody _rigidbody;
    private bool _isTurnOn = false;

    void Awake()
    {
        _input = GetComponent<HelicopterMove>();
    ...
    private void onTurnOn()
    {
        ...
        transform.Rotate(_input.X * RotationSpeed, 0f, 0f);

        if (Input.GetKey(KeyCode.LeftShift))
        {
            transform.Translate(0f, HelicopterSpeed * GoFrontSpeed, 0f);
        }
    }
